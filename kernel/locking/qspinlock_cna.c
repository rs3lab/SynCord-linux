 ...
 * preferred NUMA node. Note that certain prioritized waiters (e.g., in
...
static ulong numa_spinlock_threshold_ns = 100000000;  /* 1ms, by default */
						 ...
						 struct mcs_spinlock *node)
{
	...
	if (!cn->start_time || !intra_node_threshold_reached(cn)) {
		/*
		 * We are at the head of wait queue, no need to use
				 ...
				 struct mcs_spinlock *next)
{
	...
	if (cn->start_time != FLUSH_SECONDARY_QUEUE) {
		...
		/*
		 * We decide to flush the secondary queue;
